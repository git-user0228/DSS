...
import random
import numpy as np
from models.MGC_BR import MGCBR
...
def main():
    ...
    for lr, l2_reg, item_level_ratio, bundle_level_ratio, bundle_agg_ratio, num_layers, c_lambda, c_temp in \
        ...
        model = MGCBR(conf, dataset.graphs).to(device)
        ...
        for epoch in range(conf['epochs']):
            ...
            for batch_i, batch in pbar:
                ...
                if (batch_anchor + 1) % test_interval_bs == 0:
                    metrics = {}
                    metrics["val"], rs_val, me_val = test(model, dataset.val_loader, conf)
                    metrics["test"], rs_test, me_test = test(model, dataset.test_loader, conf)
                    best_metrics, best_perform, best_epoch = log_metrics(conf, model, metrics, run, log_path,
                                                                         ...
                                                                         best_perform, best_epoch, rs_test, me_test)
...
def log_metrics(conf, model, metrics, run, log_path, checkpoint_model_path, checkpoint_conf_path, epoch, batch_anchor,
                best_metrics, best_perform, best_epoch, rs, me):
    for topk in conf["topk"]:
    ...
    if metrics["val"]["recall"][topk_] > best_metrics["val"]["recall"][topk_] and metrics["val"]["ndcg"][topk_] > \
        ...
        users_feature, bundles_feature = rs
        IL_users_feature, BL_users_feature, _, _, AL_users_feature = users_feature
        IL_bundles_feature, BL_bundles_feature, _, AL_bundles_feature = bundles_feature
...
def test(model, dataloader, conf):
    ...
    for users, ground_truth_u_b, train_mask_u_b in dataloader:
        ...
        tmp_metrics, temp_recall, temp_ndcg, temp_indice = get_metrics(tmp_metrics, ground_truth_u_b, pred_b, conf["topk"])
        recall_5 += temp_recall
        ndcg_5 += temp_ndcg
        indice_5 += temp_indice
    ...
    return metrics, rs, [recall_5, ndcg_5, indice_5]
...
def get_metrics(metrics, grd, pred, topks):
    ...
    for topk in topks:
        ...
        tmp["recall"][topk], recall_value = get_recall(pred, grd, is_hit, topk)
        tmp["ndcg"][topk], ndcg_value = get_ndcg(pred, grd, is_hit, topk)
        if topk == 5:
            temp_recall += recall_value
            temp_ndcg += ndcg_value
            temp_indice += col_indice.tolist()
    ...
    return metrics, temp_recall, temp_ndcg, temp_indice
...
def get_recall(pred, grd, is_hit, topk):
    ...
    return [nomina, denorm], (hit_cnt / (num_pos + epsilon)).tolist()
...
def get_ndcg(pred, grd, is_hit, topk):
    ...
    return [nomina, denorm], ndcg.tolist()
